...
function parseImage(dataUrl: string) {
  ...
  if (!extension) throw new Error("INVALID_AVATAR");
...
export async function PUT(request: Request) {
  ...
  const limited = enforceRateLimit({
    key: `artist-profile:avatar:${userId}`,
    limit: 10,
    windowMs: 60 * 60_000
  });
  ...
  const storageKey = `artist-profiles/${userId}/${profileHash}.${image.extension}`;
  try {
    ...
    const settings = { ...parsed.data.settings, avatarKey: uploaded.key };
    ...
    console.error("[artist-profile-avatar] upload failed", error);
