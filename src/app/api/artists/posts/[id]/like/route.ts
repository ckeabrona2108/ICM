...
import { feedReactionSchema, toggleArtistPostLike } from "@/lib/artist-social-service";
...
export async function POST(request: NextRequest, context: { params: { id: string } }) {
  ...
  const limited = enforceRateLimit({ key: `artist-post-like:${userId}`, limit: 60, windowMs: 60 * 60_000 });
  ...
  try {
    return NextResponse.json(await toggleArtistPostLike({ prisma, postId: context.params.id, visitorId: userId, ipHash: null, reaction: parsed.data }));
  } catch (error) {
