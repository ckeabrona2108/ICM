...
import { feedReactionSchema, toggleArtistPostCommentReaction } from "@/lib/artist-social-service";
...
export async function POST(request: NextRequest, context: { params: { id: string; commentId: string } }) {
  ...
  const limited = enforceRateLimit({ key: `artist-post-comment-like:${userId}`, limit: 120, windowMs: 60 * 60_000 });
  ...
  try {
    return NextResponse.json(await toggleArtistPostCommentReaction({ prisma, postId: context.params.id, commentId: context.params.commentId, visitorId: userId, ipHash: null, reaction: parsed.data }));
  } catch (error) {
    if (error instanceof SocialInteractionBlockedError) return NextResponse.json({ error: "SOCIAL_INTERACTION_BLOCKED" }, { status: 403 });
    if (error instanceof Error && (error.message === "ARTIST_POST_NOT_FOUND" || error.message === "ARTIST_POST_COMMENT_NOT_FOUND")) return NextResponse.json({ error: "Комментарий не найден" }, { status: 404 });
