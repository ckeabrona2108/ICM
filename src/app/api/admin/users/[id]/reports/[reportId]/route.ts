...
export async function DELETE(
  ...
  const result = await adminDeleteUserFinanceReport({
    prisma,
    adminId: session.user.id,
    userId,
    reportId
  }).catch((error) => {
    console.error("[admin/users/reports] delete failed", error);
    return {
      ok: false as const,
      error: error instanceof Error && error.message ? error.message : "Не удалось удалить отчет."
    };
  });
  ...
  return NextResponse.json(
    {
      ok: true,
      reportId,
      message: "Отчет удален у пользователя."
    },
    { status: 200 }
  );
}
