...
export async function POST(
  ...
  const result = await prisma.$transaction(async (tx) => {
    ...
    const updated = await tx.payoutRequest.update({
      where: { id: payoutId },
      data: { status: PayoutRequestStatus.REJECTED, processedAt: now }
    });
    ...
    await tx.transaction.updateMany({
      ...
      data: { status: TransactionStatus.FAILED, processedAt: now }
    });
    ...
    if (!existingRefund) {
      await tx.transaction.create({
        data: {
          userId: payout.userId,
          amount: new Prisma.Decimal(Math.abs(Number(payout.amount))),
          currency: payout.currency,
          type: TransactionType.REFUND,
          status: TransactionStatus.COMPLETED,
          reference: payout.id,
          description: "Возврат по отклоненной выплате",
          processedAt: now
        }
      });
    }
